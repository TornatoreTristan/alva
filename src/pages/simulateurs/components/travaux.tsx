        ...
        <div key={categorie} className="mb-8">
          ...
          <div className="flex flex-col gap-4">
            {travaux.map((travauxItem) => (
              <div
                key={travauxItem.nom_travaux}
                className={`flex items-center p-4 border rounded-lg cursor-pointer transition-all duration-300 ${
                  selectedTravaux.includes(travauxItem.nom_travaux) ? "border-primary bg-secondary" : "border-gray-200 bg-white"
                }`}
                onClick={() => handleTravauxSelect(travauxItem.nom_travaux)}
              >
                <input
                  type="checkbox"
                  id={travauxItem.nom_travaux}
                  checked={selectedTravaux.includes(travauxItem.nom_travaux)}
                  onChange={() => handleTravauxSelect(travauxItem.nom_travaux)}
                  className="hidden"
                />
                ...
                <div className="ml-4">
                  <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${selectedTravaux.includes(travauxItem.nom_travaux) ? "border-primary" : "border-gray-400"}`}>
                    {selectedTravaux.includes(travauxItem.nom_travaux) && <div className="w-3 h-3 bg-primary rounded-full"></div>}
                  </div>
                </div>
              </div>
            ))}
