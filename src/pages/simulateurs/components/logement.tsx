...
const Logement = ({ nextStep }: { nextStep: () => void }) => {
  ...
  useEffect(() => {
    ...
    if (logementData) {
      ...
      const { selectedType, postalCode, selectedYear } = logement;
  ...
  useEffect(() => {
    ...
    updateSessionStorage("logement", logementData);
  ...
  return (
    <div className="p-8 bg-[#F9FFE6] rounded-lg border border-primary my-12">
      ...
      <div className="flex justify-center gap-8">
        ...
        >
          <input type="radio" id="maison" name="type" value="maison" checked={selectedType === "maison"} onChange={() => handleSelect("maison")} className="hidden" />
        ...
        >
          <input type="radio" id="appartement" name="type" value="appartement" checked={selectedType === "appartement"} onChange={() => handleSelect("appartement")} className="hidden" />
