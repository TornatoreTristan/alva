...
const Ventilation = ({ nextStep }: any) => {
  ...
  const [showVentilationType, setShowVentilationType] = useState(false);
          ...
          {['Oui', 'Non'].map((option) => (
            ...
            >
              <input
                ...
                name="climatisation"
                value={option}
                checked={climatisation === option}
                onChange={() => handleClimatisationSelect(option)}
                className="hidden"
              />
              ...
              <div className="ml-4">
                <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${
                  climatisation === option ? 'border-primary' : 'border-gray-400'
                }`}>
            ...
            {['Ventilation naturelle', 'VMC simple flux', 'VMC double flux'].map((option) => (
              ...
              >
                <input
                  ...
                  value={option}
                  checked={ventilationType === option}
                  onChange={() => handleVentilationTypeSelect(option)}
                  className="hidden"
                />
                <label htmlFor={option} className="flex gap-3 items-center cursor-pointer">
                  <span className="text-lg font-semibold text-gray-800">{option}</span>
                ...
                <div className="ml-4">
                  <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${
                    ventilationType === option ? 'border-primary' : 'border-gray-400'
                  }`}>
