...
const Ventilation = ({ nextStep }: any) => {
  ...
  useEffect(() => {
    ...
    if (logementData) {
      const { ventilation } = JSON.parse(logementData);
      setClimatisation(ventilation?.climatisation);
      setVentilationType(ventilation?.ventilationType);
    }
  }, []);

  useEffect(() => {
    const ventilationData = { climatisation, ventilationType };
    updateSessionStorage("ventilation", ventilationData);
  }, [climatisation, ventilationType]);
          ...
          {["Oui", "Non"].map((option) => (
            <div
              ...
              className={`flex items-center justify-between p-6 border rounded-lg cursor-pointer transition-all duration-300 ${
                climatisation === option ? "border-primary bg-secondary" : "border-gray-200 bg-white"
              }`}
              onClick={() => handleClimatisationSelect(option)}
            >
              ...
              <div className="ml-4">
                <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${climatisation === option ? "border-primary" : "border-gray-400"}`}>
                  {climatisation === option && <div className="w-3 h-3 bg-primary rounded-full"></div>}
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="flex justify-center gap-8">
            {["Ventilation naturelle", "VMC simple flux", "VMC double flux"].map((option) => (
              <div
                key={option}
                className={`flex items-center justify-between p-6 border rounded-lg cursor-pointer transition-all duration-300 ${
                  ventilationType === option ? "border-primary bg-secondary" : "border-gray-200 bg-white"
                }`}
                onClick={() => handleVentilationTypeSelect(option)}
              >
                ...
                <div className="ml-4">
                  <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${ventilationType === option ? "border-primary" : "border-gray-400"}`}>
                    {ventilationType === option && <div className="w-3 h-3 bg-primary rounded-full"></div>}
                  </div>
                </div>
              </div>
            ))}
