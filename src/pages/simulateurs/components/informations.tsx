...
const Informations = ({ nextStep, btn }: any) => {
  ...
  useEffect(() => {
    ...
    if (logementData) {
      const { informations } = JSON.parse(logementData);
      setMotif(informations?.motif);
      setEmail(informations?.email);
      setTelephone(informations?.telephone);
      setOptIn(informations?.optIn);
      if (informations?.motif && informations?.email && informations?.telephone) setShowContact(true);
    }
  }, []);
  ...
  return (
    <div className="p-8 bg-[#F9FFE6] rounded-lg border border-primary my-12">
        ...
        <div className="mb-8">
          <div className="p-4">
            <p className="text-gray-700 mb-4">
              Nous prenons la confidentialité de vos données très au sérieux. Afin de vous offrir des services personnalisés et des offres adaptées à vos besoins, nous souhaitons partager certaines de
              vos données personnelles avec nos partenaires sélectionnés. En cochant la case ci-dessous, vous consentez à ce que vos données personnelles soient partagées avec des partenaires externes
            ...
            <p className="text-gray-500 text-sm mt-4">
              Vous pouvez retirer votre consentement à tout moment en cliquant sur le lien de désabonnement dans nos emails ou en contactant notre service client à l'adresse suivante :
              [email@example.com].
            </p>
            <p className="text-gray-500 text-sm mt-4">
      ...
      <div className="text-center mt-8">
        <button
          onClick={nextStep}
          className={`px-8 py-4 font-bold rounded-lg ${isButtonDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-primary text-white cursor-pointer"}`}
