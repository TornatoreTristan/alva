...
const Configuration = ({ nextStep }: { nextStep: () => void }) => {
  ...
  useEffect(() => {
    const savedLogement = sessionStorage.getItem('logementData');
    if (savedLogement) {
      const { selectedType } = JSON.parse(savedLogement);
      setSelectedLogement(selectedType);
    }

    const savedMaisonData = sessionStorage.getItem('maisonData');
    if (savedMaisonData) {
      const { shape, adjacency, floors, surface } = JSON.parse(savedMaisonData);
      setShape(shape);
    ...
    const savedAppartementData = sessionStorage.getItem('appartementData');
    if (savedAppartementData) {
      const { emplacement, type, facades } = JSON.parse(savedAppartementData);
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem('maisonData', JSON.stringify(maisonData));
  }, [shape, adjacency, floors, surface]);

  useEffect(() => {
    const appartementData = { emplacement, type, facades };
    sessionStorage.setItem('appartementData', JSON.stringify(appartementData));
  ...
  const isNextButtonVisible = (selectedLogement === 'maison' && shape && adjacency && floors && surface) ||
    (selectedLogement === 'appartement' && emplacement && type);
        ...
        <>
                ...
                >
                  ...
                  <div className="ml-4">
                    <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${
                      shape === option ? 'border-primary' : 'border-gray-400'
                    }`}>
                ...
                {['Non mitoyenne', 'Mitoyenne sur 1 côté', 'Mitoyenne sur 2 côtés'].map((option) => (
                  ...
                  >
                    <input
                      ...
                      name="adjacency"
                      value={option}
                      checked={adjacency === option}
                      onChange={() => setAdjacency(option)}
                      className="hidden"
                    />
                    <label htmlFor={option} className="flex gap-3 items-center cursor-pointer">
                      <span className="text-lg font-semibold text-gray-800">{option}</span>
                    ...
                    <div className="ml-4">
                      <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${
                        adjacency === option ? 'border-primary' : 'border-gray-400'
                      }`}>
                ...
                {['Plain-pied', '1 étage', '2 étages ou plus'].map((option) => (
                  ...
                  >
                    <input
                      ...
                      name="floors"
                      value={option}
                      checked={floors === option}
                      onChange={() => setFloors(option)}
                      className="hidden"
                    />
                    <label htmlFor={option} className="flex gap-3 items-center cursor-pointer">
                      <span className="text-lg font-semibold text-gray-800">{option}</span>
                    ...
                    <div className="ml-4">
                      <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${
                        floors === option ? 'border-primary' : 'border-gray-400'
                      }`}>
          ...
          {floors && (
            <div className="mb-8">
              ...
              {surface && (
                <p className="text-primary mt-2">Surface habitable enregistrée: {surface} m²</p>
              )}
            </div>
          )}
        </>
              ...
              {['Rez-de-chaussée', 'Intermédiaire', 'Dernier étage'].map((option) => (
                ...
                >
                  <input
                    ...
                    name="emplacement"
                    value={option}
                    checked={emplacement === option}
                    onChange={() => setEmplacement(option)}
                    className="hidden"
                  />
                  <label htmlFor={option} className="flex gap-3 items-center cursor-pointer">
                    <span className="text-lg font-semibold text-gray-800">{option}</span>
                  ...
                  <div className="ml-4">
                    <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${
                      emplacement === option ? 'border-primary' : 'border-gray-400'
                    }`}>
              ...
              <div className="flex justify-center gap-8">
                {['Simple', 'Duplex', 'Triplex'].map((option) => (
                  ...
                  >
                    ...
                    <div className="ml-4">
                      <div className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${
                        type === option ? 'border-primary' : 'border-gray-400'
                      }`}>
          ...
          {type && (
            <div className="mb-8">
              ...
              <div className="flex justify-center items-center gap-4">
                ...
                <button
                  onClick={() => handleFacadeChange(true)}
                  className="px-4 py-2 border rounded-lg"
                >
                  +
                </button>
